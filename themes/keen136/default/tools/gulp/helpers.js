...
module.exports = {
  ...
  includePaths: ['src/theme/framework'],
        ...
        switch (type) {
          case 'styles':
            if (bundle.output.hasOwnProperty(type)) {
              ...
              stream = gulp.src(bundle.src[type], {allowEmpty: true}).pipe(_self.cssChannel(shouldRtl)()).pipe(rename({suffix: '.rtl'})).pipe(_self.outputChannel(bundle.output[type])());
              streams.push(stream);
            }
            break;
          case 'scripts':
