    ...
    export default {
        ...
        methods: {
            ...
            deleteTeam(team) {
                ...
                console.log(team);
                this.$swal({
                    title: 'Dette vil slette "' + team.name + '"',
                    ...
                    if(result.value) {
                        this.$swal({title:'Slettet korrekt', text:team.name + ' blev slettet korrekt',showConfirmButton: false, timer:1000,type:'info',position:'top'})
                        this.$store.dispatch("teams/deleteTeam",{payload: team.id});
                    } else {
                        this.$swal({title:'Annulleret', text:'Truppen blev ikke slettet.',showConfirmButton: false, timer:1000,type:'info',position:'top'})
                    }
                })
                ...
                <div class="kt-portlet__foot">
                    <div class="row align-items-center">
                        ...
                        <div class="col-6 kt-align-right">
                            ...
                            <button v-if="team.players.length > 0" v-b-tooltip.hover :title="'Du kan ikke slette en trup der indeholder spillere'" class="btn btn-secondary btn-sm"><i class="la la-trash"></i>Slet</button>
                            <button v-if="team.players.length == 0" @click="deleteTeam(team)" type="submit" :disabled="team.players.length > 0" class="btn btn-secondary btn-sm" :class="team.players.length > 0 ? 'btn-secondary' : 'btn-danger'"><i class="la la-trash"></i>Slet</button>
                        </div>
                    </div>
                </div>
            </div>
